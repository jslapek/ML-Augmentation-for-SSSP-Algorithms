...
struct Model {
    ...
    // Intentionally blank / uninformed defaults.
    // The model starts with no graph-specific prior knowledge and only adapts
    // through update() as observations arrive online.
    std::array<double, kNumFeatures> mean = {0.0, 0.0, 0.0, 0.0};
    std::array<double, kNumFeatures> scale = {1.0, 1.0, 1.0, 1.0};
    std::array<double, kNumFeatures> weights = {0.0, 0.0, 0.0, 0.0};
    double bias = 0.0;
    double sigma = 1.0;
    ...
    std::array<double, kNumFeatures> make_features(int n, int k, int t, int l) const {
        return {
    ...
    double base_log_mean(int n, int k, int t, int l) const {
    ...
    Prediction infer(const State& st, int n, int k, int t, int l) const {
    ...
    double predict_log_max_dist(int n, int k, int t, int l) const {
        return infer(State{}, n, k, t, l).log_max_dist;
    }

    double predict_max_dist(int n, int k, int t, int l) const {
        return infer(State{}, n, k, t, l).max_dist;
    }

    double predict_bound(int n, int k, int t, int l) const {
        return infer(State{}, n, k, t, l).bound;
    }

    double predict(int n, int k, int t, int l) const {
