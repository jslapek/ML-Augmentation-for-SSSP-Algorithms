#ifndef BMSSP_BOUNDED_HPP_OPT_K
#define BMSSP_BOUNDED_HPP_OPT_K
...
class batchPQ { // batch priority queue
    ...
    std::vector<elementT> pull_buf0;
    std::vector<elementT> pull_buf1;

public:
    double offset_insert = 0.0;

    batchPQ(int n = 0) : actual_value(n), where_is0(n), where_is1(n) {
        pull_buf0.reserve(16);
        pull_buf1.reserve(16);
    }
    ...
    void initialize(int M_, uniqueDistT B_) {
        ...
        pull_buf0.clear();
        pull_buf1.clear();
        if ((int)pull_buf0.capacity() < 2 * std::max(1, M_)) pull_buf0.reserve(2 * std::max(1, M_));
        if ((int)pull_buf1.capacity() < std::max(1, M_)) pull_buf1.reserve(std::max(1, M_));
    }
    ...
    std::pair<uniqueDistT, std::vector<int>> pull() {
        ...
        uniqueDistT med = selectKth(s0, M);
        std::vector<int> ret;
        ret.reserve(M);
...
private:
    ...
    void batchPrependVec(const std::vector<uniqueDistT>& values) {
        ...
        if (sz <= M) {
            ...
            for (const auto& b : values) {
                int a = std::get<2>(b);
                auto it = actual_value.find(a);
                bool exist = (it != actual_value.end());

                if (exist && it->second > b) {
                    delete_(b);
                } else if (exist) {
                    continue;
                }

                (*new_block).push_back({a, b});
                auto it_new = (*new_block).end();
                --it_new;
                where_is0[a] = {new_block, it_new};
                actual_value[a] = b;
                size_++;
            }
        ...
        for (const auto& b : values) elems.push_back({std::get<2>(b), b});
        uniqueDistT med = selectKth(elems, sz / 2);

        std::vector<uniqueDistT> less;
        std::vector<uniqueDistT> great;
        less.reserve(sz / 2 + 1);
        great.reserve(sz / 2 + 1);

        for (const auto& [a, b] : elems) {
            (void)a;
            if (b < med) {
                less.push_back(b);
            } else if (b > med) {
        ...
        great.push_back(med);
        batchPrependVec(great);
        batchPrependVec(less);
    }
};
...
public:
    ...
    void prepare_graph(bool exec_constant_degree_trasnformation = false) {
        ...
        const double logn = std::log2((double)std::max(2, total_nodes));
        k = std::max(1, (int)std::floor(std::sqrt(logn)));
        t = std::max(1, k * k);
        l = std::max(1, (int)std::ceil(logn / (double)t));

        layer_seen_epoch = 0;
        u_seen_epoch = 0;

        fast_batch_pool.clear();
        fast_batch_pool.resize(l + 1);
        for (int level_idx = 1; level_idx <= l; ++level_idx) {
            fast_batch_pool[level_idx] = std::make_unique<batchPQ<uniqueDistT>>(total_nodes);
        }

        fast_fp_pending.assign(k + 1, {});
        for (auto& bucket : fast_fp_pending) bucket.clear();
    }
...
private:
    ...
    int u_seen_epoch = 0;
    std::vector<std::unique_ptr<batchPQ<uniqueDistT>>> fast_batch_pool;
    std::vector<std::vector<int>> fast_fp_pending;
    ...
    struct SplitBatchPQ {
        ...
        double insertionOffset() const {
            return active.offset_insert;
        }
    ...
    std::pair<std::vector<int>, std::vector<int>> runFindPivotsFast(uniqueDistT B, const std::vector<int>& S) {
        ...
        if ((int)fast_fp_pending.size() < k + 1) fast_fp_pending.resize(k + 1);
        for (int depth = 0; depth <= k; ++depth) fast_fp_pending[depth].clear();
        fast_fp_pending[0].reserve(S.size());
        ...
        for (int x : S) {
            ...
            root[x] = x;
            treesz[x] = 0;
            fast_fp_pending[0].push_back(x);
        }

        for (int depth = 0; depth < k; ++depth) {
            auto& bucket = fast_fp_pending[depth];
            ...
            auto& next_bucket = fast_fp_pending[depth + 1];
            for (int x : next_bucket) layer_seen[x] = layer_seen_epoch;
    ...
    std::pair<uniqueDistT, std::vector<int>> runBMSSPFast(short int level,
        ...
        batchPQ<uniqueDistT>& D = *fast_batch_pool[level];
        D.initialize((int)batch_size, B);
        ...
        while ((long long)U.size() < quota && !D.empty()) {
            ...
            can_prepend.reserve(nw_complete.size() * 4 + current_Si.size());
            ...
            for (int x : current_Si) {
                auto dx = getDist(x);
                if (complete_B <= dx) can_prepend.emplace_back(dx);
            }

            if (!can_prepend.empty()) D.batchPrepend(can_prepend);
    ...
    PBResult resumePBMSSP(std::unique_ptr<PBMSSPState> st, wT tau, bool final_mode) {
        ...
        stats.time_full -= st->D.insertionOffset();
