#ifndef BMSSP_LAPQ_HPP_PURE
#define BMSSP_LAPQ_HPP_PURE
...
public:
    ...
    batchPQ(const batchPQ&) = delete;
    batchPQ& operator=(const batchPQ&) = delete;
    batchPQ(batchPQ&&) = delete;
    batchPQ& operator=(batchPQ&&) = delete;
...
public:
    ...
    void prepare_graph(bool exec_constant_degree_trasnformation = false) {
        ...
        for (int i = 0; i < l; ++i) {
            Ds.push_back(std::make_unique<batchPQ<uniqueDistT>>(static_cast<int>(adj.size())));
        }
    }
...
private:
    ...
    std::vector<std::unique_ptr<batchPQ<uniqueDistT>>> Ds;
