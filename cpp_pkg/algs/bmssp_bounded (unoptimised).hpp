#ifndef BMSSP_BOUNDED_HPP_UNOPT
#define BMSSP_BOUNDED_HPP_UNOPT
...
public:
    ...
    void set_threshold_schedule(std::vector<wT> thresholds_) {
    ...
    std::pair<std::vector<wT>, std::vector<int>> execute(int s) {
        ...
        if (schedule.size() == 1) {
            runBMSSPFast((short int)l, inf_dist, {s});
        } else {
            auto state = makePBMSSPState((short int)l, inf_dist, {s});

            std::size_t idx = 0;
            while (true) {
                auto res = resumePBMSSP(std::move(state), schedule[idx]);
                if (res.done) break;
...
private:
    ...
    struct SplitHeap {
        ...
        void insert(const uniqueDistT& x) {
            ...
            value[v] = x;
            if (std::get<0>(x) < tau) active.push(x);
            else dormant.push(x);
        }
    ...
    struct SplitBatchPQ {
        ...
        void initialize(int M_, uniqueDistT B_, wT tau_) {
            M = M_;
            B = B_;
            tau = tau_;
        ...
        void batchPrepend(const std::vector<uniqueDistT>& xs) {
            ...
            act.reserve(xs.size());
            for (const auto& x : xs) {
                if (std::get<0>(x) < tau) act.push_back(x);
        ...
        bool empty() {
            cleanDormant();
            return active.size() == 0 && dormant_min.empty();
        }
    ...
    std::vector<wT> normalize_schedule() const {
        ...
        if (schedule.back() < oo) schedule.push_back(oo);
        return schedule;
    }

    std::unique_ptr<PBaseCaseState> makePBaseCaseState(uniqueDistT B, int x, wT tau) {
    ...
    BaseResult resumePBaseCase(std::unique_ptr<PBaseCaseState> st, wT tau) {
        st->H.promote(tau);
    ...
    FindResult resumePFindPivots(std::unique_ptr<PFindPivotsState> st, wT tau) {
    ...
    std::unique_ptr<PBMSSPState> makePBMSSPState(short int level,
        ...
        if (level == 0) {
            ...
            st->base_state = makePBaseCaseState(B, S[0], (wT)-1);
        } else {
            st->stage = PBMSSPState::Stage::FIND_PIVOTS;
            st->find_state = makePFindPivotsState(B, S);
        }

        return st;
    }

    PBResult resumePBMSSP(std::unique_ptr<PBMSSPState> st, wT tau) {
        if (st->stage == PBMSSPState::Stage::BASECASE) {
            auto base_res = resumePBaseCase(std::move(st->base_state), tau);
            if (base_res.done) {
        ...
        if (st->stage == PBMSSPState::Stage::FIND_PIVOTS) {
            auto fp_res = resumePFindPivots(std::move(st->find_state), tau);
            if (!fp_res.done) {
            ...
            st->D.initialize((int)batch_size, st->B, tau);
        ...
        while ((long long)st->U.size() < quota) {
            if (st->stage == PBMSSPState::Stage::LOOP_CHILD) {
                auto child_res = resumePBMSSP(std::move(st->child), tau);
                if (!child_res.done) {
            ...
            st->child = makePBMSSPState((short int)(st->level - 1), st->current_Bi, st->current_Si);
