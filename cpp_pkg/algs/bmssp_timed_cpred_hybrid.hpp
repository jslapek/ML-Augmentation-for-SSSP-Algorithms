...
namespace spp_timed_cpred_hybrid {
...
private:
    ...
    inline void nextOwnerBucketEpoch() {
        ++owner_bucket_token;
        if (owner_bucket_token == std::numeric_limits<int>::max()) {
            std::fill(owner_bucket_stamp.begin(), owner_bucket_stamp.end(), 0);
            owner_bucket_token = 1;
        }
    }
    ...
    std::vector<int> scheduleRoundDedupDirect(
        const std::vector<int> &Q,
        uniqueDistT B,
        std::vector<int> &vis
    ) {
    ...
    std::vector<int> scheduleRoundHybrid(
        const std::vector<int> &Q,
        uniqueDistT B,
        std::vector<int> &vis
    ) {
        ...
        // For tiny frontiers, bucketization/sorting overhead tends to dominate.
        constexpr size_t DIRECT_DEDUP_THRESHOLD = 32;
        constexpr int SORT_BUCKET_THRESHOLD = 8;
        if (Q.size() <= DIRECT_DEDUP_THRESHOLD) {
            return scheduleRoundDedupDirect(Q, B, vis);
        }
        ...
        if (scratch_bucket_vertices.size() < Q.size()) {
            scratch_bucket_vertices.resize(Q.size());
        }

        // Pass 1: collect active owners, bucket sizes, and min distance per owner.
        for (int u : Q) {
            const int owner_u = getLocalOwnerFast(u);
            const uniqueDistT du = getLocalDistFast(u);
            ...
            if (owner_bucket_stamp[owner_u] != owner_bucket_token) {
                ...
                if (pivot_member_stamp[owner_u] == pivot_member_token) {
                    scratch_owner_pivot.push_back(owner_u);
                } else {
                    scratch_owner_nonpivot.push_back(owner_u);
                }
        ...
        if (scratch_owner_nonpivot.size() + scratch_owner_pivot.size() <= 1) {
            return scheduleRoundDedupDirect(Q, B, vis);
        }
        ...
        if (scratch_owner_nonpivot.size() > 1) {
            std::sort(scratch_owner_nonpivot.begin(), scratch_owner_nonpivot.end(), owner_cmp);
        }
        if (scratch_owner_pivot.size() > 1) {
            std::sort(scratch_owner_pivot.begin(), scratch_owner_pivot.end(), owner_cmp);
        }
        ...
        auto vertex_cmp = [&](int a, int b) {
            const uniqueDistT da = getLocalDistFast(a);
            const uniqueDistT db = getLocalDistFast(b);
            if (da != db) return da < db;
            return a < b;
        };
        ...
        auto maybe_sort_buckets = [&](const std::vector<int> &owners) {
            for (int owner_u : owners) {
                ...
                if (en - st > SORT_BUCKET_THRESHOLD) {
    ...
    std::pair<std::vector<int>, std::vector<int>> p_findPivots(
        ...
        for (int i = prefix_rounds + 1; i <= k; ++i) {
            active = scheduleRoundHybrid(active, B, vis);
            if (vis.size() > overflow_limit) {
                return {S, vis};
            }
        }
