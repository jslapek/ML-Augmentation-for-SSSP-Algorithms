...
private:
    ...
    std::vector<int> scheduleRoundOwnerBuckets(
        const std::vector<int> &Q,
        uniqueDistT B,
        std::vector<int> &vis
    ) {
        ...
        // Allocate one contiguous segment per owner, in the scheduled owner order.
        int cur_pos = 0;
        ...
        // Pass 2: materialize the buckets contiguously.
        for (int u : Q) {
            const int owner_u = getLocalOwnerFast(u);
        ...
        auto process_owner_list = [&](const std::vector<int> &owners) {
            for (int owner_u : owners) {
                ...
                for (int idx = st; idx < en; ++idx) {
                    ...
                    const uniqueDistT du = getLocalDistFast(u);
                    const int cur_owner = getLocalOwnerFast(u);
                    ...
                    for (const auto &[v, w] : adj[u]) {
                        ...
                        if (cand <= curv) {
                            setLocalStateFast(v, cand, cur_owner);
    ...
    std::pair<std::vector<int>, std::vector<int>> p_findPivots(
        ...
        for (int i = prefix_rounds + 1; i <= k; ++i) {
            active = scheduleRoundOwnerBuckets(active, B, vis);
            if (vis.size() > overflow_limit) {
                return {S, vis};
            }
        }
